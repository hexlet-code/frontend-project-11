...
export default (elements, state, posts) => {
  ...
  const addAttrElement = (element, attr) => Object.entries(attr)
    .forEach(([key, value]) => element.setAttribute(key, value));

  const createItems = (items) => items.map(({
    ...
    const isLinkVisited = (idLink) => (state.uiState.idVisitedLink.includes(idLink) ? ['fw-normal', 'link-secondary'] : ['fw-bold']);
